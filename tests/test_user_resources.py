...
def _make_group_mock(group_id: str, name: str):
    """Build a fake Okta Group whose to_dict() is what @json_response will surface."""
    group = MagicMock()
    group.id = group_id
    group.profile = MagicMock()
    group.profile.name = name
    group.to_dict.return_value = {"id": group_id, "profile": {"name": name}}
    return group
...
class TestListUserGroups:
    ...
    async def test_returns_groups_for_user(self, mock_get_client):
        """A valid user_id should return the list of groups the user belongs to, JSON-serialized."""
        client = AsyncMock()
        groups = [
        ...
        assert len(result) == 2
        assert result[0] == {"id": "00gTEST0000001", "profile": {"name": "Engineering"}}
        assert result[1] == {"id": "00gTEST0000002", "profile": {"name": "Everyone"}}

    @pytest.mark.asyncio
    async def test_result_is_json_serializable(self):
        """The tool's @json_response decorator must hand back plain JSON-native data,
        never raw Okta SDK model objects (see AGENTS.md and issue #14 / #90)."""
        import json

        with patch("okta_mcp_server.tools.user_resources.user_resources.get_okta_client") as mock_get_client:
            client = AsyncMock()
            client.list_user_groups.return_value = (
                [_make_group_mock("00gTEST0000001", "Engineering")],
                MagicMock(),
                None,
            )
    ...
    async def test_okta_api_error_is_surfaced(self, mock_get_client):
        """An Okta API error should be returned as a structured error entry in the list."""
        client = AsyncMock()
    ...
    async def test_exception_is_surfaced(self, mock_get_client):
        """An unexpected exception should be returned as a structured exception entry."""
