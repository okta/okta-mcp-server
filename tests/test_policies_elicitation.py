...
"""Tests for policy and policy-rule deletion with elicitation support."""
...
class TestDeletePolicyFallback:
    ...
    @pytest.mark.asyncio
    async def test_returns_confirmation_required(self, ctx_no_elicitation):
        result = await delete_policy(ctx=ctx_no_elicitation, policy_id=POLICY_ID)

        assert result["confirmation_required"] is True

    @pytest.mark.asyncio
    async def test_returns_fallback_on_exception(self, ctx_elicit_exception):
