...
async def delete_policy(ctx: Context, policy_id: str) -> Dict[str, Any]:
    ...
    if not outcome.used_elicitation:
        logger.info(f"Elicitation unavailable for policy {policy_id} — returning fallback")
...
async def delete_policy_rule(ctx: Context, policy_id: str, rule_id: str) -> Dict[str, Any]:
    ...
    if not outcome.used_elicitation:
        logger.info(f"Elicitation unavailable for policy rule {rule_id} — returning fallback")
        return outcome.fallback_response
